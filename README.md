...
Corretto il collegamento tra catalogo consolidato e risultati D1 di Guidonia: gli eventi con la stessa pagina fonte non vengono più deduplicati per URL, ma associati mediante la chiave univoca e arricchiti con coordinate, marker e geometrie dinamiche. Gli eventi documentali già confermati ma ancora senza coordinate mostrano “geolocalizzazione in attesa” invece di “da verificare”. La migrazione 0009 rimette in coda le posizioni automatiche di Guidonia senza eliminare gli override approvati dagli amministratori. App 0.7.4 build 84, dataset invariato alla revisione 22, Worker 0.9.8.

# A.R.I.A. 0.7.3

Ripristinato l'evento 2019 del Parco di Valle Pilella con la fonte puntuale corretta e i 50 alberi indicati nell'ordinanza. Le 61 piantumazioni AVR del 2024 sono ora separate per località e quantità: 49 a Colle Fiorito, 5 in Piazza Matteotti e 7 in Piazza Carrara. Aggiunti l'intervento 2025 di circa 200 ripiantumazioni a La Sorgente, cinque eventi 2026 per le 82 piante AVR e l'ordinanza 32/2026 relativa a potature e abbattimenti in nove tratti di Colleverde, senza inventare una quantità. Il Worker 0.9.7 geocodifica ora anche Guidonia Montecelio usando il contesto e i limiti territoriali corretti. App 0.7.3 build 83, dataset 22.

# A.R.I.A. 0.7.2

Rimosso l'evento non verificabile di Valle Pilella, il cui collegamento puntava a una pagina generale dell'archivio delle ordinanze anziché a un atto specifico. Corretto inoltre l'invio di “Segnala evento”: la chiave della fonte viene ora risolta da una funzione condivisa disponibile anche al gestore del modulo. App 0.7.2 build 82, dataset 21; Worker invariato alla 0.9.6.

# A.R.I.A. 0.7.1

La determina di Guidonia Montecelio n. 1110 del 3 ottobre 2024 è stata verificata anche nel PDF allegato. L'evento conserva lo stesso identificativo della 0.7.0 e documenta 109 alberi autorizzati all'abbattimento in 17 località o insiemi di vie, con successivo reimpianto sostitutivo previsto. La quantità resta complessiva perché l'atto non la ripartisce territorialmente; lo stato è programmato poiché l'autorizzazione non certifica l'esecuzione. Il dataset è alla revisione 20 e il Worker resta alla versione 0.9.6.
...
- **Italia** — mostra una seconda selezione **Città** con 118 capoluoghi e co-capoluoghi italiani; **Roma** è selezionata di default. I punti dei centri abitati provengono da GeoNames e sono stati verificati all'interno dei rispettivi confini comunali ISTAT 2026.
- **Europa**
...
GSOY fornisce direttamente EMNT (Extreme Min Temp), TAVG (Annual Mean Temp) ed
EMXT (Extreme Max Temp). Sono mostrate soltanto stazioni con coordinate e tutti e
tre i valori annuali.

Una stazione fisica può non essere disponibile nel solo GSOY per un certo
anno quando il servizio annuale non pubblica la tripla completa EMNT / TAVG /
EMXT. In questo caso l'app tenta il fallback osservativo descritto sotto, senza
...
Un totale pluriennale non viene ripartito artificialmente e quindi non entra nei confronti o nelle differenze fra singoli anni. Il bilancio novembre 2021–dicembre 2025 e il periodo 2024–marzo 2025 sono selezioni autonome. Per quest'ultimo la fonte non specifica il mese iniziale del 2024.
...
La procedura per forzare una scansione senza salvare il token nella cronologia del terminale è documentata in `cloudflare/temperature-proxy/README.md`. L'app e i cron automatici non conoscono e non richiedono il token amministrativo.

Il Worker corrente è la versione `0.9.8`; mantiene la scansione settimanale degli eventi, ricontrolla a rotazione gli avvisi già archiviati e smaltisce quotidianamente la coda geografica, corregge le aggregazioni termiche e interpreta correttamente indirizzi abbreviati e quantità nelle nuove schede arboree. L'app è alla versione `0.7.4` build 84 e il dataset è alla revisione 22. Gli eventi Alberi consolidati e dinamici sono ordinati per data decrescente. Gli interventi programmati con periodo già concluso sono segnalati come `Programmato · data trascorsa`, senza considerarli automaticamente eseguiti; la quantità è mostrata sotto l'icona e usa un trattino quando non è disponibile. Gli eventi privi di coordinate riportano `da verificare` e mostrano `Segnala evento`, con un modulo privo di mappa; l'amministratore può correggere i dati o escludere definitivamente l'evento dalla pubblicazione. Gli avvisi dinamici con più ubicazioni conservano tutti i luoghi e li mostrano come marker dello stesso evento. Gli utenti possono segnalare una posizione su una mappa satellitare con mirino fisso, ricerca indirizzo e ripristino della posizione iniziale; cinque clic consecutivi sulla versione aprono il pannello di revisione, protetto dal secret Cloudflare TREE_ADMIN_PASSWORD richiesto per ogni decisione. Le temperature annuali mostrano il minimo assoluto dell'anno, la media delle temperature medie giornaliere e il massimo assoluto dell'anno; minima e massima non sono più medie degli estremi giornalieri. Il catalogo geografico EEA contiene 118 capoluoghi e co-capoluoghi: tutte le coordinate sono state sostituite con i centri abitati GeoNames e verificate entro i confini comunali ISTAT 2026. La superficie EEA usa le stesse soglie cromatiche dell'elenco, quindi il colore rappresenta il valore della stazione e non la densità dei punti. In OpenAQ la select dei comuni riposiziona la mappa sul centro della città e aggiorna immediatamente le stazioni nell'area visibile. Il pulsante `Installa app` viene mostrato nella normale navigazione web e nascosto quando la PWA è avviata dalla schermata Home, anche su iOS e iPadOS. La fonte `ARPA Lazio` consente di scegliere il comune presente nei file annuali e visualizza il relativo confine amministrativo; Roma resta il valore predefinito. Il file 2025 contiene 376 comuni. I codici ARPA a otto cifre, come `12058091` per Roma, vengono normalizzati nel codice comunale ISTAT `058091`, così tutti i comuni possono popolare correttamente la select. Il selettore delle fonti usa peso tipografico normale anche quando è selezionata OpenAQ. Le select Alberi espongono soltanto annualità o periodi dotati di statistiche, aggregati o eventi documentati: sono stati rimossi il 2013–2016 e il 2022. Le annualità con eventi usano etichette compatte come `2025 eventi`; gli aggregati sono indicati soltanto dall'intervallo temporale. Le select delle altre fonti mostrano sempre il solo anno. Il dato ufficiale del 2016 copre soltanto maggio–dicembre e non viene presentato impropriamente come totale annuale; la prima annualità omogenea resta il 2017. Gli aggregati mostrano direttamente il riepilogo e il collegamento alla fonte ufficiale, senza avviso introduttivo ridondante. Se l'archivio dinamico non è raggiungibile, l'app continua a usare silenziosamente i dati consolidati senza mostrare un avviso di fallback. L'intestazione usa il logo orizzontale A.R.I.A. incluso tra gli asset offline. Il riepilogo mostra in forma compatta la data dell'ultimo aggiornamento e il numero di nuovi eventi aggiunti. Quando si seleziona un evento, le sole geometrie appartenenti a quell'evento determinano l'inquadratura; gli altri marker diventano grigi per rendere immediatamente riconoscibili le località coinvolte. Gli eventi multilocalità possono mostrare più marker collegati alla stessa quantità complessiva e vengono inquadrati insieme. L'apertura di un nuovo evento chiude automaticamente il popup precedente. In modalità Mappa il riempimento territoriale generale non apre popup; il riepilogo territoriale resta cliccabile in Confronto e Differenza. Per la fonte Alberi viene mostrata soltanto la legenda sopra la mappa, senza duplicazioni sotto la cartografia.
