(function(){
  ...
  async function catalog(){
    if(!catalogPromise)catalogPromise=fetch('data/trees.json?v=0.7.2',{cache:'no-store'}).then(response=>{
  ...
  async function proxyConfig(){
    if(!proxyConfigPromise)proxyConfigPromise=fetch('data/trees-proxy.json?v=0.7.2',{cache:'no-store'})
  ...
  async function coordinatesCatalog(){
    if(!coordinatesPromise)coordinatesPromise=fetch('data/tree-coordinates.json?v=0.7.2',{cache:'no-store'})
  ...
  async function pathsCatalog(){
    if(!pathsPromise)pathsPromise=fetch('data/tree-paths.json?v=0.7.2',{cache:'no-store'})
      .then(response=>response.ok?response.json():{events:{}})
      .catch(()=>({events:{}}));
    return pathsPromise
  }
