...
async function loadVersion(){
  const [appVersion,dataVersion]=await Promise.all([
    fetch('version.json?v=0.5.3',{cache:'no-store'}).then(r=>r.json()),
    fetch('data/version.json?v=0.5.3',{cache:'no-store'}).then(r=>r.json())
  ]);
...
async function boot(){
  ...
  if('serviceWorker'in navigator){
    navigator.serviceWorker.register('./service-worker.js?v=0.5.3')
      .then(reg=>reg.update())
      .catch(console.error)
  }
}
